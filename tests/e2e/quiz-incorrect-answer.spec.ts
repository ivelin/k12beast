...
test.describe("Quiz Incorrect Answer Feedback V3", () => {
  test("should show encouragement and readiness for incorrect answer", async ({ page }) => {
    ...
    await expect(page.locator("text=Good effort! Let's try another one.")).toBeVisible({ timeout: 15000 });
    await expect(page.locator("text=Test Readiness:")).toBeVisible({ timeout: 15000 });
    await expect(page.locator("text=75%")).toBeVisible({ timeout: 15000 });
  });
});
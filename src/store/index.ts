...
const useAppStore = create<AppState>((set, get) => ({
  ...
  handleSubmit: async (problem, imageUrls, images) => {
    ...
    try {
      ...
      if (images.length > 0) {
        ...
        if (!uploadResponse.ok) throw new Error((await uploadResponse.json() as { error: string }).error || "Failed to upload images");
      ...
      addMessage({ role: "user", content: problem, renderAs: "markdown", experimental_attachments: uploadedImageUrls.map((url, i) => ({ name: images[i]?.name || `Image ${i + 1}`, url })) });

      const token = document.cookie.split("; ").find(row => row.startsWith("supabase-auth-token="))?.split("=")[1];
      const headers: HeadersInit = { "Content-Type": "application/json", "x-session-id": sessionId || "" };
      if (token) headers["Authorization"] = `Bearer ${token}`;

      const res = await fetch("/api/tutor", { method: "POST", headers, body: JSON.stringify({ problem, images: uploadedImageUrls }) });
      const lessonContent: string = await res.text();
  ...
  handleExamplesRequest: async () => {
    ...
    try {
      ...
      const res = await fetch("/api/examples", { method: "POST", headers, body: JSON.stringify({ problem, images: imageUrls }) });
      ...
      addMessage({ role: "assistant", content: `<p><strong>Example:</strong> ${data.problem}</p><p><strong>Solution:</strong></p><ul>${data.solution.map(s => `<li><strong>${s.title}:</strong> ${s.content}</li>`).join("")}</ul>`, renderAs: "html" });
    } catch (err) {
  ...
  handleQuizSubmit: async () => {
    ...
    try {
      ...
      const res = await fetch("/api/quiz", { method: "POST", headers, body: JSON.stringify({ problem, images: imageUrls }) });
      ...
      addMessage({ role: "assistant", content: `<p><strong>Quiz:</strong></p><p>${data.problem}</p><ul>${data.options.map(o => `<li>${o}</li>`).join("")}</ul>`, renderAs: "html" });
    } catch (err) {
  ...
  handleValidate: async (answer, quiz) => {
    ...
    try {
      ...
      const res = await fetch("/api/validate", { method: "POST", headers, body: JSON.stringify({ sessionId, problem: quiz.problem, answer }) });
      const data = await res.json() as QuizFeedback;
      if (!res.ok) throw new Error((data as any).error || "Failed to validate quiz");

      const isCorrect = answer === quiz.correctAnswer;
      const readinessConfidence = isCorrect ? quiz.readiness.confidenceIfCorrect : quiz.readiness.confidenceIfIncorrect;
      const readinessPercentage = Math.round(readinessConfidence * 100);

      const motivationalMessage = readinessPercentage >= 90 ? "You're doing amazing! You're very likely to ace your big test!" :
        readinessPercentage >= 70 ? "Great progress! You're on track to do well on your big test. Keep practicing!" :
        readinessPercentage >= 50 ? "You're making progress! Let's keep working to boost your confidence for the big test." :
        "Let's keep practicing! More effort will help you succeed on your big test.";

      set({ quizAnswer: answer, quizFeedback: data });
      addMessage({ role: "assistant", content: `<p><strong>Feedback:</strong></p><p><strong>Your Answer:</strong> ${answer}</p><p>${data.commentary}</p>${data.solution ? `<p>${data.solution.map(s => `<strong>${s.title}:</strong> ${s.content}`).join("</p><p>")}</p>` : ""}<p><strong>Options:</strong></p><ul>${quiz.options.map(o => `<li>${o}${o === answer ? " (Your answer)" : ""}${o === quiz.correctAnswer ? " (Correct answer)" : ""}</li>`).join("")}</ul><p><strong>Test Readiness:</strong></p><div class="readiness-container"><div class="readiness-bar" style="width: ${readinessPercentage}%"></div></div><p>${readinessPercentage}% - ${motivationalMessage}</p>`, renderAs: "html" });
    } catch (err) {
  ...
  reset: () => set({ step: 'problem', sessionId: null, problem: '', images: [], imageUrls: [], lesson: null, examples: null, quiz: null, error: null, loading: false, quizAnswer: '', quizFeedback: null, messages: [], hasSubmittedProblem: false }),
}));

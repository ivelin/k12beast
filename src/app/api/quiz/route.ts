...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `You are a highly professional K12 tutor. Generate a quiz problem similar to "${originalProblem}" for a student around ${age} years old in grade ${grade} with ${skillLevel} skill level. Consider the student's performance: ${performanceSummary}. If the student has been performing well, make the quiz slightly more challenging. If they have been struggling, provide an easier quiz. Return JSON: {"problem": "Quiz problem text", "answerFormat": "multipleChoice|text|image", "options": ["option1", "option2", ...] (if multipleChoice), "correctAnswer": "correct answer"}
